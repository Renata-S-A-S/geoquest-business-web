...
describe('createReward', () => {
  ...
  it('siembra el stock restante con el total', async () => {
    const created = await createReward(businessId, { ...INPUT, stockTotal: 7 })
    const rewards = await getRewards(businessId)
  ...
  it('acepta stock ilimitado como null', async () => {
    const created = await createReward(businessId, { ...INPUT, stockTotal: null })
    const rewards = await getRewards(businessId)
  ...
  it('rechaza con el error de axios cuando el backend responde 400', async () => {
    ...
    await expect(createReward(businessId, INPUT)).rejects.toMatchObject({
